...
LayoutCompiler.prototype.build = function() {
  ...
  this.inputPaths.forEach(function(currentPath) {
    ...
    var layouts = collectLayouts(currentPath, pathInfo);
...
function collectLayouts(dirPath, pathInfo) {
  var layouts = [];
  ...
  dir.forEach(function(name) {
    ...
    if (stat && stat.isDirectory()) {
      if (name === 'layouts') {
        layouts.push(collectLayout(dirPath, pathInfo));
      } else {
        layouts = layouts.concat(collectLayouts(f, pathInfo));
      }
    }
  });
...
function collectLayout(dirPath, pathInfo) {
